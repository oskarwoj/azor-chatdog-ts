 ...
 * Alias for printWarning - both use yellow for visual consistency.
 */
export const printHelp = printWarning;
...
export function displayHelp(sessionId: string): void {
	...
	printHelp(
		'  /switch [ID]      - Przełącza na istniejącą sesję (interaktywne wyszukiwanie gdy bez ID).',
	);
	...
	printHelp(
		'  /audio [--lang=KOD] - Odczytuje ostatnią odpowiedź asystenta (domyślnie: pl-PL).',
	);
	printHelp('\n  /assistant list   - Wyświetla listę dostępnych asystentów.');
	printHelp('  /assistant switch <ID> - Przełącza na innego asystenta.');
}
...
export function displayFinalInstructions(sessionId: string): void {
	...
	printInfo(
		`Aby kontynuować tę sesję (ID: ${sessionId}) później, użyj komendy:`,
	);
